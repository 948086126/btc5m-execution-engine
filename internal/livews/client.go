...
func Dial(rawURL string, timeout time.Duration) (*Client, error) {
	...
	if u.Scheme != "wss" && u.Scheme != "ws" { return nil, fmt.Errorf("unsupported websocket scheme %q", u.Scheme) }
	host := u.Hostname()
	port := u.Port()
	if port == "" { if u.Scheme == "wss" { port = "443" } else { port = "80" } }
	...
	if resp.StatusCode != http.StatusSwitchingProtocols { if resp.Body != nil { _ = resp.Body.Close() }; conn.Close(); return nil, fmt.Errorf("websocket upgrade status %d", resp.StatusCode) }
...
func (c *Client) ReadText(maxPayload int) ([]byte, error) {
	for {
		...
		case 0x8:
			...
			return nil, io.EOF
		case 0x9:
			if err := c.writeControl(0xA, payload); err != nil { return nil, err }
		case 0xA:
			continue
		default:
...
func writeFrame(w io.Writer, opcode byte, payload []byte, masked bool) error {
	...
	n := len(payload)
	switch {
	case n <= 125: hdr.WriteByte(maskBit | byte(n))
	case n <= 65535: hdr.WriteByte(maskBit | 126); _ = binary.Write(&hdr, binary.BigEndian, uint16(n))
	default: hdr.WriteByte(maskBit | 127); _ = binary.Write(&hdr, binary.BigEndian, uint64(n))
	}
	out := payload
	if masked {
		key := [4]byte{}
		...
		for i := range out { out[i] ^= key[i%4] }
	}
	if _, err := w.Write(hdr.Bytes()); err != nil { return err }
	_, err := w.Write(out)
	return err
}
...
func readFrame(br *bufio.Reader, maxPayload int) (byte, []byte, error) {
	...
	var mask [4]byte
	if masked { if _, err := io.ReadFull(br, mask[:]); err != nil { return 0, nil, err } }
	payload := make([]byte, int(n))
	if _, err := io.ReadFull(br, payload); err != nil { return 0, nil, err }
