...
func main() {
	...
	flag.Parse()
	cfg.Source=strings.ToUpper(strings.TrimSpace(cfg.Source)); cfg.Duration=duration
	if cfg.URL=="" {
		switch cfg.Source {
		case "M2": cfg.URL="wss://fstream.binance.com/ws/btcusdt@bookTicker"
		case "SPOT": cfg.URL="wss://stream.binance.com:9443/ws/btcusdt@bookTicker"
		case "PM": cfg.URL="wss://ws-subscriptions-clob.polymarket.com/ws/market"
		default: fatal("--source must be m2, spot, or pm")
		}
	}
	if cfg.RunID=="" || cfg.OutPath=="" { fatal("--run-id and --out are required") }
	if cfg.Source=="PM" && cfg.PlaneID=="" { fatal("PM requires --plane A or B") }
	if err:=livesource.Run(cfg);err!=nil{ fatal(err.Error()) }
